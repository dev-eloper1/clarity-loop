...
|----------|--------|----------|-------------|
| DESIGN_SYSTEM.md | `{docsRoot}/specs/DESIGN_SYSTEM.md` | Yes | Token catalog, component catalog, traceability |
| UI_SCREENS.md | `{docsRoot}/specs/UI_SCREENS.md` | No | Screen inventory, component usage |
| DESIGN_TASKS.md | `{docsRoot}/specs/DESIGN_TASKS.md` | No | Implementation plan |
| DESIGN_PROGRESS.md | `{docsRoot}/designs/DESIGN_PROGRESS.md` | No | Design decisions and user approvals |
| PRD | System docs | Yes | Source of truth for UI-facing features and requirements |
| Architecture doc | System docs | No | Tech stack context for design-to-code alignment |
| Previous design reviews | `{docsRoot}/reviews/design/DESIGN_REVIEW_*.md` | No | Prior reviews for tracking whether findings were addressed |
| Codebase | Project source files | No | For design-to-code alignment checks |
| DECISIONS.md | `docs/DECISIONS.md` | No | Log design-to-PRD misalignment decisions and naming conflict resolutions |
| Design review output path | `{docsRoot}/reviews/design/DESIGN_REVIEW_[YYYY-MM-DD].md` | Yes | Where the design review is written |
...
**Verify**: PRD features extracted. Design artifacts loaded. Previous reviews checked.

**On failure**: If DESIGN_SYSTEM.md doesn't exist, stop and tell the user to run `/cl-designer setup`.

### Phase 2: Four-Dimension Analysis

5. **Dimension 1: Design vs. System Docs** -- Does the design accurately represent what the system docs describe?
...
6. **Dimension 2: Design vs. Code** -- If a codebase exists, does the design align with code conventions?
...
7. **Dimension 3: Visual Quality (Gestalt + Accessibility)** -- Does the design follow established visual perception principles and accessibility standards?
   ...
   **Accessibility constraints (WCAG 2.2):**
   ...
   - Target sizes: interactive elements >= 24x24px. Button height >= 36px.
...
8. **Dimension 4: Internal Consistency** -- Is the design system consistent with itself?
...
**Verify**: All four dimensions analyzed. MCP-specific checks performed where available.

### Phase 3: Produce the Design Review

9. Create the review at:
   ```
...
**Verify**: Design review file written with all four dimensions and correct verdict.

### Phase 4: Update Tracking

11. If the review reveals design gaps that trace to missing PRD content, flag it:
    "The design review found gaps that may need PRD updates. Consider a research cycle
    for: [list]."

12. If the review resulted in design-to-PRD misalignment decisions, naming conflict
...
13. Tell the user where the review file is and summarize the verdict. If issues were found,
    be specific about what needs fixing -- the user will likely re-run the relevant cl-designer
    mode to address them.

**Verify**: Tracking updated. User informed of verdict and next steps.

## Report

Output: Design review at `{docsRoot}/reviews/design/DESIGN_REVIEW_[YYYY-MM-DD].md`

### On success
```
DESIGN-REVIEW: APPROVE | Issues: 0
```

### On failure
```
DESIGN-REVIEW: CHANGES NEEDED | Issues: N
```
