...
|----------|--------|----------|-------------|
| Approved proposal | `docs/proposals/P-NNN-slug.md` | Yes | Source of truth for what should have changed (Change Manifest is the verification checklist) |
| Latest review | `docs/reviews/proposals/REVIEW_P-NNN_v*.md` | Yes | Most recent review to understand what was approved and any conditions |
| All system docs | `docs/system/` | Yes | Read fresh -- manifest is stale because docs were just modified |
| Proposal tracker | `docs/PROPOSAL_TRACKER.md` | Yes | Updated with verified status and merge date |
| DECISIONS.md | `docs/DECISIONS.md` | No | Log discrepancies where system doc version was preferred over proposal |
| Verify output path | `docs/reviews/proposals/VERIFY_P-NNN.md` | Yes | Where the verification file is written |
...
### Phase 2: Five-Part Verification

4. **Part A: Application Completeness** -- Walk through each concrete change the proposal describes and verify it landed in the
...
**Verify**: Every Change Manifest item has been checked and categorized.

5. **Part B: Fidelity Check** -- For each applied change, verify the system doc version faithfully represents the proposal:
...
8. **Part E: Code Alignment Check** -- Verify that code-related claims in the merged sections still match the actual codebase.
   ...
   |-------|--------------|----------|--------|----------|
   | pgvector dimension | ARCHITECTURE.md section Memory | 768-dim | 1536-dim in schema.sql | Warning |
...
**Verify**: All five verification parts completed. Application status, fidelity, cross-doc consistency, collateral damage, and code alignment all assessed.

### Phase 3: Produce the Verification File

9. Create the verification file at:
   ```
...
**Verify**: Verification file written with all required sections populated.

### Phase 4: Update Tracking and Manifest

11. Update `docs/PROPOSAL_TRACKER.md` -- set status to `verified`, record the merge date.

12. If verification found discrepancies where the system doc version was preferred over the proposal (or vice versa), log a Decision entry in `docs/DECISIONS.md` explaining what diverged and which version was kept.

13. The manifest will be auto-regenerated by the PostToolUse hook since system docs changed.

**Verify**: Tracker updated. Decision entries logged for any discrepancies.

14. After writing the file, tell the user where it is and summarize the verdict. If the merge
...
### Phase 5: Design Nudge

15. After verification completes, check if system docs reference UI features. Search the PRD
    (and other system docs) for terms like "interface", "dashboard", "UI", "frontend", "screen",
    "page", "view", "component", "layout".

16. If UI-related terms are found, check whether design artifacts exist: look for
    `{docsRoot}/designs/` directory and `{docsRoot}/specs/DESIGN_SYSTEM.md`.

17. If the system docs describe UI features but no design artifacts exist, suggest:
...
Output: Verification file at `docs/reviews/proposals/VERIFY_P-NNN.md`

### On success
```
VERIFY: PASS | Changes: N/N applied
```

### On failure
```
VERIFY: FAIL | Missing: N | Drift: N
```
