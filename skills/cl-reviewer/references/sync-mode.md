...
|----------|--------|----------|-------------|
| System docs | `docs/system/` | Yes | Source of claims to verify against code |
| Codebase | Project source files | Yes | The actual code to verify claims against |
| Git ref (for git-diff mode) | User-provided `--since <ref>` | No | Limits scope to code changed since this ref |
| DECISIONS.md | `docs/DECISIONS.md` | No | Log decisions about how to handle confirmed drift |
| Sync report path | `{docsRoot}/reviews/audit/SYNC_[YYYY-MM-DD].md` | Yes | Where the sync report is written |
...
**Full scan** (`/cl-reviewer sync`): Verify all extractable claims in system docs against
the codebase. More thorough but slower -- reads all system docs and performs targeted code
searches for each claim.

**Git-diff** (`/cl-reviewer sync --since <ref>`): Only check claims related to code areas
that changed since `<ref>`. Faster and more targeted -- useful for incremental checks.

Examples:
- `/cl-reviewer sync` -- full scan
- `/cl-reviewer sync --since main~10` -- last 10 commits on main
- `/cl-reviewer sync --since v1.0.0` -- since a tag
- `/cl-reviewer sync --since 2026-01-15` -- since a date (git resolves this)

## Workflow

### Phase 1: Extract Claims from System Docs

1. Read the system docs (all for full scan, or relevant ones for git-diff mode) and identify
   statements that make verifiable claims about the codebase.

2. **Claim types to extract:**
...
**Verify**: Claims extracted and catalogued with types. Aspirational and abstract claims excluded.

**On failure**: If no system docs exist, stop and inform the user.

### Phase 2: Analyze Code State
...
5. **For git-diff mode:**
   ...
   3. Filter claims from Phase 1 to only those related to changed domains
   4. This reduces the verification surface significantly

6. **For full scan mode:**
...
**Verify**: Code state analyzed for all in-scope claims.

### Phase 3: Cross-Reference

7. For each claim, verify it against the code and categorize the finding:
...
**Verify**: Every claim categorized. Conservative approach applied to drift categorization.

### Phase 4: Produce Sync Report

9. Create the report at:
   ```
...
10. Use this structure:
    ...
    **Code areas analyzed**: [list of directories/files checked]
    ...
    1. **Immediate corrections**: Confirmed drift items that can be fixed via `/cl-reviewer correct`
...
**Verify**: Report written with all sections. Counts in summary match individual findings.

### Phase 5: Integration with Other Modes

11. The sync report feeds into the pipeline like other audit artifacts:
    - **Confirmed drift with obvious fixes** -> `/cl-reviewer correct SYNC_DATE.md`
      (correction mode, using the sync report as the source finding)
    - **Confirmed drift needing design decisions** -> `/cl-researcher research "topic"`
...
12. **Decision logging for confirmed drift**: When the user decides how to handle confirmed
...
**Verify**: Integration paths communicated to user. Decision logging guidance provided.
...
Output: Sync report at `{docsRoot}/reviews/audit/SYNC_[YYYY-MM-DD].md`

### On success
```
SYNC: COMPLETE | Files: N synced
```

### On failure
```
SYNC: DRIFT | Files: N out of sync
```
