...
1. **Check for stale `.pipeline-authorized` marker** — If `docs/system/.pipeline-authorized`
   ...
   be resolved before starting new work. Use `/doc-reviewer` to clean up or finish."
...
6. Tell the user: "Research approved. You can run `/doc-researcher structure` to plan the
   document structure, or `/doc-researcher proposal` to go directly to proposal generation."
...
that's structured for the doc-reviewer skill. The proposal must include:
