...
1. **Specs exist** — Read `{docsRoot}/specs/.spec-manifest.md`. If it doesn't exist:
   "No specs found. Run `/doc-spec-gen generate` first to create implementation specs."
   Stop.

2. **Spec review** — Check if any spec review output exists (from `/doc-spec-gen review`).
   If not: "Specs haven't been reviewed for consistency. Consider running `/doc-spec-gen
...
3. Read `{docsRoot}/specs/DESIGN_TASKS.md` if it exists (from ui-designer build-plan mode).
...
Tell the user:
"Implementation plan ready. {N} tasks across {M} areas. Run `/implementer run` to start
processing the queue, or `/implementer status` for an overview."
