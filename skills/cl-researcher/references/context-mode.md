...
1. **Freshness check (soft, configurable)**
   ...
   - Who triggers: cl-implementer start mode (pre-check), cl-implementer spec (gate check)
...
2. **Version mismatch (hard)**
   ...
   - Who triggers: cl-implementer start mode, cl-implementer spec gate check, manual invocation
...
3. **Failure-based (strongest)**
   ...
   - Who triggers: cl-implementer run mode (Step 4 classification)
