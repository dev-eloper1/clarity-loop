...
Write `.pipeline-authorized` (project root):
...
1. Remove `.pipeline-authorized` (project root)
...
3. Create `.pipeline-authorized` (project root) with `operation: bootstrap, source: import`
4. For each mapping:
...
- `.pipeline-authorized` (project root) created and removed during process
