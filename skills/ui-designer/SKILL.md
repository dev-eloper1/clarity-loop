...
System docs exist (PRD with UI features)
  |
  v
/ui-designer setup         # Detect MCP + design discovery conversation
  |
  v
/ui-designer tokens        # Design tokens + reusable component library
...
/doc-reviewer design-review    # Validate designs against PRD + internal consistency
  |
  v
/doc-spec-gen generate         # Tech specs (can reference design artifacts)
```
...
1. **Check for stale `.pipeline-authorized` marker** — If `docs/system/.pipeline-authorized`
   ...
   Resolve this before designing — use `/doc-reviewer` to clean up."
...
3. **Design state check** — Read `{docsRoot}/designs/DESIGN_PROGRESS.md` if it exists.
   ...
   `/ui-designer setup` to start fresh, or try to reconstruct state from existing artifacts
   ...
   - If setup is complete but tokens are pending: "Found existing design progress. Setup
     was completed on [date] using [MCP path]. Ready for `/ui-designer tokens`."
   - If tokens are complete: "Design system exists with [N] components. Ready for
     `/ui-designer mockups` or `/ui-designer build-plan`."
   - If mockups are complete: "Mockups exist for [N] screens. Ready for
     `/ui-designer build-plan`."
...
- **Design review is separate.** This skill creates; `/doc-reviewer design-review` validates.
