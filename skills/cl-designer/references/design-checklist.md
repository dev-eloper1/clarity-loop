...
|----------|--------|----------|-------------|
| DESIGN_SYSTEM.md | `{docsRoot}/specs/` | Yes (tokens checklist) | Token catalog + component catalog to verify |
| UI_SCREENS.md | `{docsRoot}/specs/` | Yes (mockups checklist) | Screen inventory + behavioral contracts to verify |
| DESIGN_PROGRESS.md | `{docsRoot}/designs/` | Yes | Approval entries and phase statuses |
| DECISIONS.md | `{docsRoot}/decisions/` | No | Auto-default decisions logged with `[auto-default]` tag |
| ux.autoDefaults | `.clarity-loop.json` | No | Controls which tiers auto-proceed: `"none"`, `"tier3"` (default), `"tier2-3"` |
| PRD | `docs/system/` or equivalent | Yes | Cross-reference for feature coverage checks |

## Guidelines

1. Tier 1 (Must Confirm) items stop the pipeline and require explicit user approval.
2. Tier 2 (Batch Review) items are generated in batch; the user reviews the set and flags issues.
3. Tier 3 (Auto-proceed) items are generated with sensible defaults and logged to DECISIONS.md with `[auto-default]` tag.
4. The `ux.autoDefaults` setting controls the auto-proceed boundary: `"none"` (most conservative), `"tier3"` (default), `"tier2-3"` (most aggressive).
5. Every Tier 3 auto-proceed decision is logged with `[auto-default]` tag, the default value, source, and timestamp.
6. The user can search DECISIONS.md for `[auto-default]` and override any decision at any time.
7. If behavioral/accessibility gaps remain after the checklist, present what's missing and let the user decide whether to proceed.

## Process

### Phase 1: Checkpoint Tiering
...
**Checkpoint**: Tiering system understood and `ux.autoDefaults` configuration read.
...
| 13 | Target sizes met | Interactive elements >= 24x24px. Button height >= 36px. Icon buttons >= 32x32px. Checkbox/radio controls >= 20x20px with padding to 24x24px touch area. | 3 |
...
**Checkpoint**: Tokens checklist complete with results presented to user.
...
**Checkpoint**: Mockups checklist complete with results presented to user.

## Output

- **Primary artifact**: Checklist results (pass/fail per item) presented to user for final approval
- **Additional outputs**: DECISIONS.md entries for auto-proceeded items with `[auto-default]` tag
